...
-- Ids de citas creadas por el público. No chocan con los ids que genera el
-- cliente (Date.now()*1000 + azar, del orden de 10^15).
...
begin
  if not exists (select 1 from pg_policies where schemaname = 'public'
                 and tablename = 'business_hours' and policyname = 'business_hours_all') then
    create policy business_hours_all on business_hours
      using (is_member(business_id)) with check (is_member(business_id));
  end if;

  if not exists (select 1 from pg_policies where schemaname = 'public'
                 and tablename = 'time_block' and policyname = 'time_block_all') then
    create policy time_block_all on time_block
      using (is_member(business_id)) with check (is_member(business_id));
  end if;

  if not exists (select 1 from pg_policies where schemaname = 'public'
                 and tablename = 'booking_policy' and policyname = 'booking_policy_all') then
